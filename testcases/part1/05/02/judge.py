...
try:
    ...
    for i in range(NR_PUBLISHERS):
        if not wait_for_output(publishers[i], "[PUBLISHER SYNC END]", MAX_TIMEOUT, f"publisher{i}"):
            kill_all_procs()
            sys.exit(JUDGE_WA)

    for i in range(NR_PUBLISHERS):
        publishers[i] = run_command([f"{BINARY_DIR}/pub", CHANNEL_NAME, f"fakefile{NR_PUBLISHERS+i}", str(nr_msgs_per_pub[i] * 4096)], f"publisher{NR_PUBLISHERS+i}")
        if not wait_for_output(publishers[i], PJOIN, MAX_TIMEOUT, f"publisher{NR_PUBLISHERS+i}"):
            kill_all_procs()
            sys.exit(JUDGE_WA)
